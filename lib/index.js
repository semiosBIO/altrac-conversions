...
var litersToAcreFeet = function litersToAcreFeet(value, precision) {
  var returnValue = 0;
...
var litersToGallons = function litersToGallons(value, precision) {
  var returnValue = 0;
...
var litersToUserPreference = function litersToUserPreference(value, userPreference, unitType, precision) {
  var returnValue = 0;
  ...
  if (!Number.isNaN(value) && Number(value) > 0) {
    ...
    } else if ( userPreference === 'acreFeet') {
      returnValue = round((Number(value) / 1233481.8553199936), precision);
...
function valueCalculator(formula, value, context, precision) {
  ...
  switch (formula) {
    ...
    case 'litersToGallons':
      returnValue = litersToGallons(value, userPreference, unitType, precision);
      break;

    case 'litersToUserPreference':
      returnValue = litersToUserPreference();
      break;
...
module.exports = {
  ...
  litersToAcreFeet: litersToAcreFeet,
