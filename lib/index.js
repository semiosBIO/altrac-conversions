...
var isBoolean = function isBoolean(value) {
  return typeof value === 'boolean' || isNumber(value) || typeof value === 'string' && /^(true|false|t|f|on|off)$/i.test(value);
};
...
function valueCalculator(formula, value, context, precision) {
...
var displayFormula = function displayFormula(formula, multiplier, precision, context, _valueKey, readingCurrent, readingLast, physical, mapValues) {
  ...
  switch (formulaValue) {
    ...
    case 'pumpStopped':
      {
        var _valueKey2 = valueKey,
            run = _valueKey2.run;
        var _valueKey3 = valueKey,
            signal = _valueKey3.signal;
        var isRun = valueCalculator(run.formula, readingCurrent[run.valueKey] / (run.multiplier || 1), run.context || '', run.precision || 0);
        var isSignal = valueCalculator(signal.formula, readingCurrent[signal.valueKey] / (signal.multiplier || 1), signal.context || '', signal.precision || 0);

        if (isBoolean(isRun) && isBoolean(isSignal)) {
          var runB = toBoolean(isRun);
          var signalB = toBoolean(isSignal);

          switch (formula) {
            case 'pumpRunning':
              returnValue = runB && signalB;
              break;
